...
function normalizeEndpoint(raw) {
	...
	try {
		...
		return {
			ok: false,
			code: "invalid-endpoint",
			message: `endpoint is not a valid URL: ${trimmed}`
		};
	}
	...
	if (url.hostname === "") return {
		ok: false,
		code: "invalid-endpoint",
		message: `endpoint has no host: ${trimmed}`
	};
	return {
		ok: true,
...
*/
function sanitizeTargets(raw, max = MAX_TARGETS) {
...
*/
function activeTarget(targets, activeTargetId) {
...
async function refreshAttach(input) {
	...
	if (input.remoteEnabled === false) return cache.patch({
		...suggested,
		status: "no-active",
...
const EGO_HELP_INDEX = {
	overview: "CDP 浏览器代理：结构化浏览器工具。导航/交互/观察/表单/网络/等待/键鼠皆有专项工具，另提供 bcdp_help(本索引)、bcdp_doctor(体检)、bcdp_cli/bcdp_script(自由脚本逃生舱)。分类见: tools / navigate / observe / input / keyboard-mouse / form / wait / network / login / script / doctor。用 `topic` 查询，或直接给工具名。",
...
*/
function normalizeProbeState(target) {
	return {
		...target,
...
function apply(ctx, config = {}) {
	...
	const cfg = {
		...
		get cdpTargets() {
			return resolveConfig(bridge.source()).cdpTargets;
		},
		get activeTargetId() {
...
//#endregion
export { Config, apply, cdpAttachError, createActiveSpaceTracker, decideCdpAttach, findChromeBinary, inject, name, resolveEgoEnv, runWithStaleSpaceRetry, shouldReapBrowser };