...
var en = {
	...
	runtimeArgs: "Extra runtime CLI args",
	runtimeArgsHint: "Appended to the vendored runtime argv. Takes effect on the next bcdp_* call. Blocked: --status/--stop/--open/--spaces/--help (they exit before the heredoc runs). Use --headless via the headless env instead.",
	chromeArgs: "Extra Chrome launch args",
	chromeArgsHint: "Appended to the Chrome launch argv. Takes effect on the next browser cold start (the browser is a singleton — stop the browser from the panel or restart DSH to relaunch). Blocked: --user-data-dir/--remote-debugging-port/--remote-allow-origins/--headless/--no-startup-window/--proxy-server (use EGO_LINUX_PROXY for the proxy).",
	save: "Save",
	saving: "Saving…",
	discard: "Discard",
	unsaved: "Unsaved",
	...
	jevUrl: "JEV base URL",
	jevUrlHint: "No path. Empty disables the jev hop entirely.",
	jevKey: "JEV key",
	jevKeyHint: "Bearer key. Empty skips the jev hop instead of sending a request that would be rejected.",
	jevModel: "JEV model",
	layaUrl: "Laya base URL",
	layaUrlHint: "The Laya sidecar serves port 8000. Port 7789 is a different tool and will not answer.",
	layaKey: "Laya key",
	layaKeyHint: "REQUIRED — laya-api has no anonymous access, so a keyless call is a guaranteed 401. Empty skips the hop.",
	layaModel: "Laya model",
	judgePrefer: "Hop order",
	judgePreferHint: "Comma separated, e.g. jev,laya,rule. Unknown names are dropped; the terminal refusal hop always remains.",
	jevChunkSize: "Candidates per round",
	jevChunkSizeHint: "Also selects the probability threshold bucket, so raising it tightens the gate.",
	jevMaxImageBytes: "Frame byte budget",
	jevMaxImageBytesHint: "0 = unbounded (recommended: a full-page JPEG measured ~137 KiB).",
	jevHistoryLimit: "History depth",
	jevArchiveImage: "Embed image in archive",
	jevArchiveImageOn: "Embedded (larger bundles)",
	jevArchiveImageOff: "Metadata only",
...
var zh = {
	...
	runtimeArgs: "运行时附加参数",
	runtimeArgsHint: "追加到内置运行时 argv。下一次 bcdp_* 工具调用即生效。禁止：--status/--stop/--open/--spaces/--help（会在 heredoc 执行前退出）。--headless 请走 headless 环境变量。",
	chromeArgs: "Chrome 启动附加参数",
	chromeArgsHint: "追加到 Chrome 启动 argv。仅在浏览器下次冷启动时生效（浏览器是单例常驻——从面板关闭浏览器或重启 DSH 后才会重新启动）。禁止：--user-data-dir/--remote-debugging-port/--remote-allow-origins/--headless/--no-startup-window/--proxy-server（代理请用 EGO_LINUX_PROXY）。",
	save: "保存",
	saving: "保存中…",
	discard: "放弃",
	unsaved: "未保存",
	...
	jevUrl: "JEV 地址",
	jevUrlHint: "不要带路径。留空即彻底关闭 jev 这一跳。",
	jevKey: "JEV 密钥",
	jevKeyHint: "Bearer 密钥。留空时**跳过** jev，而不是发一个注定被拒的请求。",
	jevModel: "JEV 模型",
	layaUrl: "Laya 地址",
	layaUrlHint: "laya sidecar 服务在 8000 端口；7789 是别的工具，连不上。",
	layaKey: "Laya 密钥",
	layaKeyHint: "**必填** —— laya-api 无匿名分支，缺 key 必然 401。留空则跳过该跳。",
	layaModel: "Laya 模型",
	judgePrefer: "跳序",
	judgePreferHint: "逗号分隔，如 jev,laya,rule。无法识别的名字会被丢弃；终点的拒绝跳永远保留。",
	jevChunkSize: "每轮候选上限",
	jevChunkSizeHint: "同时决定概率阈值分桶，调大也会同步收紧闸门。",
	jevMaxImageBytes: "帧字节预算",
	jevMaxImageBytesHint: "0 = 不限（推荐：实测整页 JPEG 约 137 KiB）。",
	jevHistoryLimit: "历史深度",
	jevArchiveImage: "归档内嵌图片",
	jevArchiveImageOn: "内嵌（归档包更大）",
	jevArchiveImageOff: "仅元数据",
...
function initialSettingsState() {
	return {
		...
		draft: {
			...
			layaKey: "",
			layaModel: "laya",
			judgePrefer: "jev,laya,rule",
			jevChunkSize: "20",
			jevMaxImageBytes: "0",
			jevHistoryLimit: "5",
			jevArchiveImage: false,
			jevStepBudget: "20",
			jevWallMs: "120000"
		},
		ffmpegStatus: {
	...
	fetch("/bcdp/api/get", {
		...
		self.store.update(function(s) {
			...
			s.draft = {
				...
				jevModel: config.jevModel || "jev",
				layaUrl: config.layaUrl || "http://127.0.0.1:8000",
				layaKey: config.layaKey || "",
				layaModel: config.layaModel || "laya",
				judgePrefer: config.judgePrefer || "jev,laya,rule",
				jevChunkSize: String(config.jevChunkSize ?? 20),
				jevMaxImageBytes: String(config.jevMaxImageBytes ?? 0),
				jevHistoryLimit: String(config.jevHistoryLimit ?? 5),
				jevArchiveImage: config.jevArchiveImage === true,
				jevStepBudget: String(config.jevStepBudget ?? 20),
				jevWallMs: String(config.jevWallMs ?? 12e4)
			};
...
function mapLinkRows(raw) {
	...
	return raw.map(function(l) {
		...
		if (row.kind === EGO_CLI_KIND) {
			row.cliPath = row.cliPath || "";
			row.endpoint = "";
		} else {
			row.kind = "cdp";
...
EgoBrowserSettingsController.prototype._commitCdp = function(patch) {
	if (patch.links !== void 0) this.stagedComplex.links = patch.links;
	if (patch.activeTargetId !== void 0) this.stagedComplex.activeTargetId = patch.activeTargetId;
	if (patch.cdpMode !== void 0) this.stagedComplex.cdpMode = patch.cdpMode;
	this.store.update(function(s) {
		if (patch.links !== void 0) s.draft.links = patch.links;
		if (patch.activeTargetId !== void 0) s.draft.activeTargetId = patch.activeTargetId;
		if (patch.cdpMode !== void 0) s.draft.cdpMode = patch.cdpMode;
		s.dirty = true;
		s.applyState = { kind: "idle" };
	});
};
...
EgoBrowserSettingsController.prototype.addTarget = function(kind) {
	var links = (this.store.getSnapshot().draft.links || []).slice();
...
EgoBrowserSettingsController.prototype.moveTarget = function(id, dir) {
	var targets = (this.store.getSnapshot().draft.links || []).slice();
	var index = targets.findIndex(function(t) {
		return t.id === id;
	});
	if (index === -1) return;
...
EgoBrowserSettingsController.prototype.editTarget = function(id, field, value) {
	var targets = (this.store.getSnapshot().draft.links || []).map(function(t) {
		if (t.id !== id) return t;
...
EgoBrowserSettingsController.prototype.setActiveTarget = function(id) {
	var target = (this.store.getSnapshot().draft.links || []).find(function(t) {
		return t.id === id;
	});
...
EgoBrowserSettingsController.prototype.probeTarget = function(id) {
	...
	function applyOutcome(o, errorText) {
		var targets = (self.store.getSnapshot().draft.links || []).map(function(t) {
			if (t.id !== id) return t;
			var next = Object.assign({}, t);
	...
	fetch("/bcdp/api/cdp-probe", {
		...
		applyOutcome(parsed && parsed.ok && parsed.value ? parsed.value.outcome : null, "");
	}).catch(function(e) {
		applyOutcome(null, e instanceof Error ? e.message : String(e));
	});
};
...
EgoBrowserSettingsController.prototype._doSave = function() {
	...
	for (var ck in this.stagedComplex) if (Object.prototype.hasOwnProperty.call(this.stagedComplex, ck)) patch[ck] = this.stagedComplex[ck];
...
function BrowserLinksBlock(props) {
	...
	useEffect(function() {
		...
		function poll() {
			fetch("/bcdp/api/cdp-status", {
				...
				if (alive && j && j.ok) setAttach(j.value.attach);
			}).catch(function() {});
		}
		poll();
		var timer = setInterval(poll, 5e3);
		return function() {
			alive = false;
			clearInterval(timer);
		};
	}, []);
	function sourceLabel(attach$1) {
		if (!attach$1 || attach$1.status !== "ready") return "";
		if (attach$1.endpointSource === "cli") return " · " + t("cdpStatusCli");
		if (attach$1.endpointSource === "cli-launch") return " · " + t("cdpStatusCliLaunch");
		if (attach$1.endpointSource === "local" || attach$1.endpointSource === "local-fallback") return " · " + t("cdpStatusLocal");
		return "";
	}
	...
	for (var i = 0; i < links.length; i++) if (links[i].id === activeTargetId) {
		activeTarget = links[i];
		break;
	}
	var hasCliLink = false;
	for (var k = 0; k < links.length; k++) if (links[k].kind === EGO_CLI_KIND) {
		hasCliLink = true;
		break;
	}
	var showActiveBadge = !!activeTarget && activeTarget.enabled;
	return h("div", { className: "dsh-ego-card__ffmpeg" }, h("div", { className: "dsh-ego-card__ffmpeg-title" }, t("cdpSectionTitle")), h("div", { className: "dsh-ego-card__hint" }, t("cdpSectionIntro")), h(SettingsField, {
		...
		options: [
			...
			{
				value: "remote",
				label: t("cdpModeRemote")
			},
			{
				value: "local",
				label: t("cdpModeLocal")
			}
		],
		disabled: busy,
		onEdit: function(v) {
			controller.editCdpMode(v);
		}
	}), showActiveBadge ? h("div", {
		className: "dsh-ego-card__ffmpeg-status",
		role: "status"
	}, t("cdpActive") + ": " + (activeTarget.label || activeTarget.endpoint || t("linkKindCli")) + " · " + statusLabel(attach ? attach.status : "idle") + sourceLabel(attach) + (attach && attach.latencyMs ? " · " + t("cdpLatency") + " " + attach.latencyMs + "ms" : "") + (attach && attach.code ? " · " + attach.code : "")) : h("div", { className: "dsh-ego-card__notice" }, t("cdpActivateHint")), links.length === 0 ? h("div", { className: "dsh-ego-card__hint" }, t("cdpNoTargets")) : h("div", { className: "dsh-ego-card__cdp-list" }, links.map(function(tg, idx) {
		var isActive = tg.id === activeTargetId;
		var isCli = tg.kind === EGO_CLI_KIND;
		return h("div", {
			key: tg.id,
			className: "dsh-ego-card__cdp-target" + (isActive ? " dsh-ego-card__cdp-target--active" : "")
		}, h("div", { className: "dsh-ego-card__field-row" }, h("label", {
			className: "dsh-ego-card__radio",
			title: t("cdpActive")
		}, h("input", {
			type: "radio",
			checked: isActive,
			disabled: !tg.enabled || busy,
			onChange: function() {
				controller.setActiveTarget(tg.id);
			}
		}), h("span", { className: "dsh-ego-card__unit" }, idx + 1 + ".")), h("label", {
			className: "dsh-ego-card__radio",
			title: t("cdpEnabled")
		}, h("input", {
			type: "checkbox",
			...
			disabled: busy,
			onChange: function(e) {
				controller.editTarget(tg.id, "enabled", e.target.checked);
			}
		}), h("span", { className: "dsh-ego-card__unit" }, t("cdpEnabled"))), h("span", { className: "dsh-ego-card__unit dsh-ego-card__cdp-kind" }, isCli ? t("linkKindCli") : t("linkKindCdp"))), isCli ? h(SettingsField, {
			id: "cdp-clipath-" + tg.id,
			label: t("cliPath"),
			hint: t("cliPathHint"),
			value: tg.cliPath || "",
			placeholder: "/usr/local/bin/ego-browser",
			disabled: busy,
			onEdit: function(v) {
				controller.editTarget(tg.id, "cliPath", v);
			}
		}) : h(SettingsField, {
			id: "cdp-endpoint-" + tg.id,
			label: t("cdpEndpoint"),
			...
			disabled: busy,
			onEdit: function(v) {
				controller.editTarget(tg.id, "endpoint", v);
			}
		}), isCli ? h(SettingsField, {
			id: "cdp-sdkpath-" + tg.id,
			label: t("cliUseSdkPath"),
			hint: t("cliUseSdkPathHint"),
			value: tg.useSdkPath ? "true" : "false",
			options: [{
				value: "false",
				label: t("cliUseSdkPathOff")
			}, {
				value: "true",
				label: t("cliUseSdkPathOn")
			}],
			disabled: busy,
			onEdit: function(v) {
				controller.editTarget(tg.id, "useSdkPath", v === "true");
			}
		}) : null, h(SettingsField, {
			id: "cdp-label-" + tg.id,
			label: t("cdpLabel"),
			value: tg.label || "",
			disabled: busy,
			onEdit: function(v) {
				controller.editTarget(tg.id, "label", v);
			}
		}), h(SettingsField, {
			id: "cdp-note-" + tg.id,
			label: t("cdpNote"),
			...
			disabled: busy,
			onEdit: function(v) {
				controller.editTarget(tg.id, "note", v);
			}
		}), !isActive && tg.probeStatus && tg.probeStatus !== "unknown" ? h("div", {
			className: "dsh-ego-card__ffmpeg-status",
			role: "status"
		}, (tg.probeStatus === "ok" ? t("cdpStatusReady") : t("cdpStatusUnreachable")) + (tg.probeLatencyMs ? " · " + t("cdpLatency") + " " + tg.probeLatencyMs + "ms" : "") + (tg.probeError ? " · " + t("cdpError") + ": " + tg.probeError : "")) : null, h("div", { className: "dsh-ego-card__ffmpeg-actions" }, isCli ? null : h("button", {
			type: "button",
			className: "dsh-ego-card__btn",
			disabled: busy || !tg.endpoint,
			onClick: function() {
				controller.probeTarget(tg.id);
			}
		}, t("cdpProbe")), h("button", {
			type: "button",
			className: "dsh-ego-card__btn",
			disabled: busy || idx === 0,
			onClick: function() {
				controller.moveTarget(tg.id, -1);
			}
		}, t("cdpMoveUp")), h("button", {
			type: "button",
			className: "dsh-ego-card__btn",
		...
		}, t("cdpMoveDown")), h("button", {
			type: "button",
			className: "dsh-ego-card__btn",
			disabled: busy,
			onClick: function() {
				controller.removeTarget(tg.id);
			}
		}, t("cdpRemove"))));
	})), h("div", { className: "dsh-ego-card__ffmpeg-actions" }, h("button", {
		type: "button",
		className: "dsh-ego-card__btn dsh-ego-card__btn--primary",
		disabled: busy,
		onClick: function() {
			controller.addTarget("cdp");
		}
	}, t("cdpAdd")), h("button", {
		type: "button",
		className: "dsh-ego-card__btn",
		...
		onClick: function() {
			controller.addTarget(EGO_CLI_KIND);
		}
	}, t("cdpAddCli"))));
}
function EgoBrowserCard(props) {
	...
	if (!open) body = null;
		...
		body = h("div", { className: "dsh-ego-card__body" }, !state.writable ? h("p", {
			...
			id: "plugin-config-dsh-browser-cdp-idletimeout",
			...
			id: "plugin-config-dsh-browser-cdp-backend",
			...
			id: "plugin-config-dsh-browser-cdp-profile",
			...
			id: "plugin-config-dsh-browser-cdp-cdpfps",
			label: t("cdpFps"),
			...
			id: "plugin-config-dsh-browser-cdp-cdpquality",
			label: t("cdpQuality"),
			...
			id: "plugin-config-dsh-browser-cdp-cdpwidth",
			label: t("cdpMaxWidth"),
			...
			id: "plugin-config-dsh-browser-cdp-backstop",
			...
			id: "plugin-config-dsh-browser-cdp-fffps",
			label: t("ffmpegFps"),
			...
			id: "plugin-config-dsh-browser-cdp-ffwidth",
			...
			id: "plugin-config-dsh-browser-cdp-ffbitrate",
			...
			id: "plugin-config-dsh-browser-cdp-encoder",
			...
			id: "plugin-config-dsh-browser-cdp-ffpath",
			label: t("ffmpegPath"),
			...
			id: "plugin-config-dsh-browser-cdp-ghmirror",
			...
			id: "plugin-config-dsh-browser-cdp-ego-cli-args",
			label: t("runtimeArgs"),
			value: state.draft.runtimeArgs,
			hint: t("runtimeArgsHint"),
			placeholder: "--sdk-path /path/to/harness.js",
			disabled: busy,
			onEdit: function(v) {
				controller.edit("runtimeArgs", v);
			}
		}), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-remote-enabled",
			label: t("remoteEnabled"),
			hint: t("remoteEnabledHint"),
			value: state.draft.remoteEnabled ? "true" : "false",
			options: [{
				value: "true",
				label: t("remoteEnabledOn")
			}, {
				value: "false",
				label: t("remoteEnabledOff")
			}],
			disabled: busy,
			onEdit: function(v) {
				controller.edit("remoteEnabled", v === "true");
			}
		}), h("div", { class: "plugin-config-section" }, h("div", { class: "plugin-config-section-title" }, t("judgeTitle")), h("div", { class: "plugin-config-section-hint" }, t("judgeHint")), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-jev-url",
			label: t("jevUrl"),
			hint: t("jevUrlHint"),
			value: state.draft.jevUrl,
			placeholder: "https://jev.example",
			disabled: busy,
			onEdit: function(v) {
				controller.edit("jevUrl", v);
			}
		}), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-jev-key",
			label: t("jevKey"),
			hint: t("jevKeyHint"),
			value: state.draft.jevKey,
			disabled: busy,
			onEdit: function(v) {
				controller.edit("jevKey", v);
			}
		}), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-jev-model",
			label: t("jevModel"),
			value: state.draft.jevModel,
			disabled: busy,
			onEdit: function(v) {
				controller.edit("jevModel", v);
			}
		}), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-laya-url",
			label: t("layaUrl"),
			hint: t("layaUrlHint"),
			value: state.draft.layaUrl,
			placeholder: "http://127.0.0.1:8000",
			disabled: busy,
			onEdit: function(v) {
				controller.edit("layaUrl", v);
			}
		}), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-laya-key",
			label: t("layaKey"),
			hint: t("layaKeyHint"),
			value: state.draft.layaKey,
			disabled: busy,
			onEdit: function(v) {
				controller.edit("layaKey", v);
			}
		}), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-laya-model",
			label: t("layaModel"),
			value: state.draft.layaModel,
			disabled: busy,
			onEdit: function(v) {
				controller.edit("layaModel", v);
			}
		}), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-judge-prefer",
			label: t("judgePrefer"),
			hint: t("judgePreferHint"),
			value: state.draft.judgePrefer,
			placeholder: "jev,laya,rule",
			disabled: busy,
			onEdit: function(v) {
				controller.edit("judgePrefer", v);
			}
		}), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-jev-chunk",
			label: t("jevChunkSize"),
			hint: t("jevChunkSizeHint"),
			value: state.draft.jevChunkSize,
			numeric: true,
			narrow: true,
			min: 1,
			max: 255,
			...
			disabled: busy,
			onEdit: function(v) {
				controller.edit("jevChunkSize", v);
			}
		}), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-jev-image-bytes",
			label: t("jevMaxImageBytes"),
			hint: t("jevMaxImageBytesHint"),
			value: state.draft.jevMaxImageBytes,
			numeric: true,
			narrow: true,
			min: 0,
			step: 1024,
			disabled: busy,
			onEdit: function(v) {
				controller.edit("jevMaxImageBytes", v);
			}
		}), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-jev-history",
			label: t("jevHistoryLimit"),
			value: state.draft.jevHistoryLimit,
			numeric: true,
			narrow: true,
			min: 0,
			max: 20,
			...
			disabled: busy,
			onEdit: function(v) {
				controller.edit("jevHistoryLimit", v);
			}
		}), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-jev-archive-image",
			label: t("jevArchiveImage"),
			value: state.draft.jevArchiveImage ? "true" : "false",
			options: [{
				value: "true",
				label: t("jevArchiveImageOn")
			}, {
				value: "false",
				label: t("jevArchiveImageOff")
			}],
			disabled: busy,
			onEdit: function(v) {
				controller.edit("jevArchiveImage", v === "true");
			}
		}), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-jev-step-budget",
			label: t("jevStepBudget"),
			value: state.draft.jevStepBudget,
			numeric: true,
			narrow: true,
			min: 1,
			max: 200,
			...
			disabled: busy,
			onEdit: function(v) {
				controller.edit("jevStepBudget", v);
			}
		}), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-jev-wall-ms",
			label: t("jevWallMs"),
			value: state.draft.jevWallMs,
			numeric: true,
			narrow: true,
			unit: t("msUnit"),
			min: 1e3,
			...
			disabled: busy,
			onEdit: function(v) {
				controller.edit("jevWallMs", v);
			}
		})), h(SettingsField, {
			id: "plugin-config-dsh-browser-cdp-chrome-args",
			label: t("chromeArgs"),
		...
		}), h(LoginImportBlock, { t }), h(BrowserLinksBlock, {
			t,
			controller,
			links: state.draft.links,
			activeTargetId: state.draft.activeTargetId,
			cdpMode: state.draft.cdpMode,
			busy
...
function apply(ctx) {
	ctx.effect(() => ctx.locale.register(SETTINGS_NS, {
		zh,
		en
	}), "dsh-browser-cdp: settings dictionaries");
	ctx.effect(() => {
...
function EgoBrowserTab(props) {
	...
	if (!currentSpace) body = h$1("div", { className: "dsh-ego-side-body" }, h$1("div", { className: "dsh-ego-side-empty" }, h$1("div", null, wt("noActivePages")), h$1("div", { style: { fontSize: "11px" } }, wt("noActiveHint"))));
		...
		}, state.pick === "failed" ? wt("pickFailed") : state.pick === "on" ? wt("picking") : wt("pickMode")), h$1("button", {
			className: "dsh-ego-side-back",
			type: "button",
