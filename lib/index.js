...
const EGO_HELP_INDEX = {
	...
	judge: "JEV/Laya 判定（阶段 10）：把「截图 + 编号 DOM + 意图」组装成判定请求；判定器只能回传编号，绝不给选择器（幻觉选择器在结构上不可能）。四件套：bcdp_jev_status（先跑这个：判定链可用性 + 配置体检，不发任何请求）/ bcdp_jev_frame（采一帧，看判定器会看到什么）/bcdp_jev_ask（组装请求体；dryRun 默认 true，先看清楚再发）/ bcdp_jev_run（跑循环，返回逐步 trace）。降级链 jev -> laya -> rule -> refuse：不可用的跳**跳过不调用**，每一跳的跳过与失败都记进 trace，绝不静默回落；refuse 是结果不是异常。laya-api 强制鉴权且无匿名分支 —— 缺 key 时该跳直接跳过，而不是发出去收 401。阈值一律用选中项概率 top（不是 confidence：后者是归一化熵，2 选 1 与 20 选 1 不可比），并按候选数分桶。设置面板键：jevUrl/jevKey/jevModel、layaUrl/layaKey/layaModel（laya 端口 8000，不是 7789）、judgePrefer、jevChunkSize、jevMaxImageBytes、jevHistoryLimit、jevArchiveImage、jevStepBudget、jevWallMs。"
};
...
function buildHistorySection(history, limit, excluded) {
	...
	if (recent.length === 0) lines.push("(no actions taken yet — this is the first judgement on this frame)");
		...
		lines.push(`- ${step$1.action}${target} ${step$1.ok ? "ok" : "FAILED"}: ${step$1.note}`);
	}
	if (excluded.size > 0) {
...
function buildRound(input) {
	...
	if (input.round === "pick" && chunk === null) throw new Error(`pick round asked for chunk ${chunkIndex} but the plan has ${plan.chunkTotal} chunk(s)`);
	const excluded = new Set(input.excluded ?? []);
	...
	const offered = input.round === "control" ? [] : (chunk?.nodes ?? input.frame.dom.nodes).filter((node) => !excluded.has(node.n));
	const questions = input.round === "control" ? controlQuestions(canScroll(input.frame)) : pickQuestions(offered);
...
async function runLoop(input) {
	...
	while (true) {
		...
		if (controlAnswer === null) return finish("exhausted", "budget exhausted: judge", excluded, steps, degraded, ledger, "judge");
		...
		if (outcome === null) return finish("exhausted", "budget exhausted: judge", excluded, steps, degraded, ledger, "judge");
		...
		if (outcome.kind !== "pick") {
			...
			if (consecutiveUnclear >= unclearLimit) return finish("stuck", `candidate answer unusable ${consecutiveUnclear}×`, excluded, steps, degraded, ledger);
			continue;
		}
...
*/
async function walkChunks(ledger, effects, frame, intent, history, excluded, pipe) {
	...
	for (let index = 1; index <= chunkTotal; index += 1) {
		if (!ledger.check("chunksPerCapture").ok) return null;
		const start = (index - 1) * chunkSize;
		if (frame.dom.nodes.slice(start, start + chunkSize).filter((node) => !excludedSet.has(node.n)).length === 0) continue;
...
async function ask(ledger, effects, round) {
	if (!ledger.check("judge").ok) return null;
	if (!ledger.check("wallMs").ok) return null;
	ledger.spend("judge");
	return effects.judge({
	...
	*/
	...
	reg(defineTool({
		...
		execute: async (_args, exec) => {
			...
			if (judge.layaKey === "") lines.push("", "note: laya had no key, so that hop is SKIPPED (laya-api has no anonymous branch — a keyless call is a 401).");
			return {
				ok: true,
				text: lines.join("\n")
			};
