	...
	fetch("/bcdp/api/get", {
		...
		self.store.update(function(s) {
			...
			s.draft = {
				...
				cdpTargets: Array.isArray(config.cdpTargets) ? config.cdpTargets.map(function(t) {
...
EgoBrowserSettingsController.prototype._commitCdp = function(patch) {
	if (patch.cdpTargets !== void 0) this.stagedComplex.cdpTargets = patch.cdpTargets;
	...
	this.store.update(function(s) {
		if (patch.cdpTargets !== void 0) s.draft.cdpTargets = patch.cdpTargets;
...
EgoBrowserSettingsController.prototype.addTarget = function() {
	var targets = (this.store.getSnapshot().draft.cdpTargets || []).slice();
	targets.push({
		id: genCdpTargetId(),
		label: "",
		endpoint: "",
...
EgoBrowserSettingsController.prototype.removeTarget = function(id) {
	...
	var patch = { cdpTargets: (draft.cdpTargets || []).filter(function(t) {
		return t.id !== id;
	}) };
...
EgoBrowserSettingsController.prototype.editTarget = function(id, field, value) {
	var targets = (this.store.getSnapshot().draft.cdpTargets || []).map(function(t) {
		if (t.id !== id) return t;
...
EgoBrowserSettingsController.prototype.setActiveTarget = function(id) {
	var target = (this.store.getSnapshot().draft.cdpTargets || []).find(function(t) {
		return t.id === id;
	});
...
EgoBrowserSettingsController.prototype.probeTarget = function(id) {
	...
	function applyOutcome(o, errorText) {
		var targets = (self.store.getSnapshot().draft.cdpTargets || []).map(function(t) {
			if (t.id !== id) return t;
			var next = Object.assign({}, t);
...
function CdpTargetsBlock(props) {
	...
	for (var i = 0; i < targets.length; i++) if (targets[i].id === activeTargetId) {
	...
	return h("div", { className: "dsh-ego-card__ffmpeg" }, h("div", { className: "dsh-ego-card__ffmpeg-title" }, t("cdpSectionTitle")), h("div", { className: "dsh-ego-card__hint" }, t("cdpSectionIntro")), h(SettingsField, {
		...
		className: "dsh-ego-card__ffmpeg-status",
		role: "status"
	}, t("cdpActive") + ": " + (activeTarget.label || activeTarget.endpoint) + " · " + statusLabel(attach ? attach.status : "idle") + (attach && attach.latencyMs ? " · " + t("cdpLatency") + " " + attach.latencyMs + "ms" : "") + (attach && attach.code ? " · " + attach.code : "")) : h("div", { className: "dsh-ego-card__notice" }, t("cdpActivateHint")), targets.length === 0 ? h("div", { className: "dsh-ego-card__hint" }, t("cdpNoTargets")) : h("div", { className: "dsh-ego-card__cdp-list" }, targets.map(function(tg, idx) {
		...
		return h("div", {
		...
		}), h("span", { className: "dsh-ego-card__unit" }, t("cdpEnabled")))), h(SettingsField, {
			...
			className: "dsh-ego-card__ffmpeg-status",
			role: "status"
		}, (tg.probeStatus === "ok" ? t("cdpStatusReady") : t("cdpStatusUnreachable")) + (tg.probeLatencyMs ? " · " + t("cdpLatency") + " " + tg.probeLatencyMs + "ms" : "") + (tg.probeError ? " · " + t("cdpError") + ": " + tg.probeError : "")) : null, h("div", { className: "dsh-ego-card__ffmpeg-actions" }, h("button", {
			type: "button",
			className: "dsh-ego-card__btn",
			...
			type: "button",
			className: "dsh-ego-card__btn",
			disabled: busy || idx === targets.length - 1,
			onClick: function() {
				controller.moveTarget(tg.id, 1);
			}
...
function EgoBrowserCard(props) {
	...
	if (!open) body = null;
		...
		}), h(LoginImportBlock, { t }), h(CdpTargetsBlock, {
			t,
			controller,
			targets: state.draft.cdpTargets,
