      ...
      handler: async (reqRaw: unknown, resRaw: unknown) => {
        ...
        try {
          ...
          if (method === 'get') {
            ...
            // 0.1.7: config writes belong to the declarative settings form
            // (configForms). The gateway only serves runtime capability routes.
            writeJson(res, 410, { ok: false, error: { code: 'GONE', message: 'config writes moved to the 0.1.7 settings form' } })
