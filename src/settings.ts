// src/settings.ts — host-side bridge between the `dsh-browser-cdp` settings
...
export const SETTINGS_NAMESPACE = 'dsh-browser-cdp' as SettingsNamespace
    ...
    if (!scope) {
      try {
        ...
        if (
          ...
          throw error
        ctx.logger?.('dsh-browser-cdp')?.warn(
