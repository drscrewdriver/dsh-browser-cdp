...
function deliverPickToConversation(ctx, element, action) {
	try {
		...
		if (action !== "comment" && action !== "send") return {
			ok: false,
			code: "bad-action"
		};
		...
		if (snap && snap.phase && snap.phase !== "plain") return {
			ok: false,
			code: "phase-not-plain",
			phase: snap.phase
		};
...
function createPickControl(onState, deliver) {
	...
	function applyState(state) {
		...
		if (state.lastPick && state.lastAction && state.picks !== deliveredPicks && typeof deliver === "function") {
			...
			if (result && result.ok) emit("picked", result.code === "submitted" ? wt("pickSent") : wt("pickDrafted"));
