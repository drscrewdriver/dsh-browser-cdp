...
async function probeEndpoint(endpoint, options = {}) {
	...
	try {
		const response = await (options.fetchVersion ?? ((target, init) => fetch(target, { signal: init.signal })))(url, { signal });
		...
		return {
			...
			message: timedOut ? `no answer from ${url} within ${timeoutMs}ms` : `endpoint unreachable: ${err?.message ?? String(error)}`,
			latencyMs: elapsed()
		};
	}
}
